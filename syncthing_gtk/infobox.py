...
class InfoBox(Gtk.Container):
    ...
    def init_header(self):
        ...
        hbox.override_background_color(
            Gtk.StateFlags.NORMAL, Gdk.RGBA(*self.color))
        eb.add(hbox)
        # Update stuff
    ...
    def do_draw(self, cr):
        ...
        # Half of border is rendered outside of widget
        cr.set_line_width(self.border_width * 2)
        cr.stroke()
        ...
        # Header
        cr.set_source_rgba(*self.real_color)
        cr.rectangle(self.border_width / 2.0, 0, allocation.width -
                     self.border_width, header_al.height + (2 * self.border_width))
        cr.fill()
    ...
    def hilight_timer(self, *a):
        ...
        if self.hilight and self.hilight_factor < 1.0:
            self.hilight_factor = min(
                1.0, self.hilight_factor + COLOR_CHANGE_STEP)
        elif not self.hilight and self.hilight_factor > 0.0:
            self.hilight_factor = max(
                0.0, self.hilight_factor - COLOR_CHANGE_STEP)
        else:
            self.timer_enabled = False
        self.recolor()
    ...
    def recolor(self, *a):
        ...
        try:
            self.header.get_children()[0].override_background_color(
                Gtk.StateFlags.NORMAL, gdkcol)
        except IndexError:
    ...
    def set_status(self, t, percentage=0.0):
        if percentage > 0.0 and percentage < 1.0:
            ...
            self.status.set_markup('<span color="white" %s>%s (%.f%%)</span>' % (
                self.app.config["infobox_style"],
                t, percent))
            log.debug("%s state changed to %s (%s%%)",
                      self.str_title, t, percent)
        else:
            ...
            if key not in svg_cache:
                if self.dark_color is not None:
                    ...
                    svg_source = svg_source.replace(
                        'fill:rgb(0%,0%,0%)', 'fill:rgb(100%,100%,100%)')
                    svg = Rsvg.Handle.new_from_data(svg_source.encode("utf-8"))
                else:
                    # Load svg directly
                    svg = Rsvg.Handle.new_from_file(
                        os.path.join(self.app.iconpath, icon))
    ...
    def add_value(self, key, icon, title, value="", visible=True):
        ...
        for w in self.value_widgets[key]:
            w.override_background_color(
                Gtk.StateFlags.NORMAL, Gdk.RGBA(*self.background))
