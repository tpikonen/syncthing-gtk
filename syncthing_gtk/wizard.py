...
class Wizard(Gtk.Assistant):
    def __init__(self, gladepath="/usr/share/syncthing-gtk",
        ...
        self.st_configfile = os.path.join(
        ...
        if IS_WINDOWS:
            self.set_icon_list([GdkPixbuf.Pixbuf.new_from_file(
    ...
    def search(self):
        ...
        try:
            ...
            if IS_WINDOWS:
                ...
                cb = Gtk.CheckButton(
                    _("_Download Syncthing binary"), use_underline=True)
                cb.connect("toggled", lambda cb, *
                           a: self.parent.set_page_complete(page, cb.get_active()))
                page.attach(cb,	0, 2, 2, 1)
                # Attach [ ] Autoupdate checkbox
                cb = Gtk.CheckButton(
                    _("Auto_update downloaded binary"), use_underline=True)
                cb.connect("toggled", lambda cb, *
                           a: self.parent.config.set("st_autoupdate", cb.get_active()))
...
class DownloadSTPage(Page):
    ...
    def init_page(self):
        ...
        self.label = WrappedLabel(
            "<b>" + _("Downloading Syncthing daemon.") + "</b>")
...
class GenerateKeysPage(Page):
    ...
    def start_binary(self):
        ...
        """
        self.parent.output_line(
            "syncthing-gtk: Configuration directory: '%s'" % (self.parent.st_configdir,))
        # Create it, if needed
        try:
            os.makedirs(self.parent.st_configdir)
        except Exception as e:
            self.parent.output_line(
                "syncthing-gtk: Failed to create configuration directory")
            self.parent.output_line("syncthing-gtk: %s" % (str(e),))
        # Run syncthing -generate
        self.parent.output_line(
            "syncthing-gtk: Syncthing configuration directory: %s" % (self.parent.st_configdir,))
        self.process = DaemonProcess(
            [self.parent.config["syncthing_binary"], '-generate=%s' % self.parent.st_configdir])
        self.process.connect('line', lambda proc,
                             line: self.parent.output_line(line))
...
class HttpSettingsPage(Page):
    ...
    def cb_stuff_changed(self, *a):
        ...
        values_ok = self.rb_localhost.get_active() or (len(self.tx_username.get_text().strip())
                                                       and len(self.tx_password.get_text().strip()))
        ...
        if values_ok:
            ...
            self.parent.syncthing_options["user"] = str(
                self.tx_username.get_text())
            self.parent.syncthing_options["password"] = str(
                self.tx_password.get_text())
...
class SaveSettingsPage(Page):
    ...
    def check_port(self, port):
        ...
        try:
            ...
            self.parent.output_line(
                "syncthing-gtk: port %s is not available" % (port,))
    ...
    def save_settings(self):
        ...
        """
        # Generate API key
        self.apikey = "".join(random.choice(
        ...
        try:
            ...
            self.parent.output_line("syncthing-gtk: %s" %
        ...
        try:
            ...
            self.ct_textnode(xml, gui, "user",
                             self.parent.syncthing_options["user"])
