...
class Daemon(GObject.GObject, TimerManager):
    ...
    def _request_folder_data(self, folder_id):
        id_enc = urllib.parse.quote(folder_id)
    ...
    def _syncthing_cb_system(self, data):
        ...
        if self._my_id != data["myID"]:
            ...
            version = get_header(data[HTTP_HEADERS],
                                 b"X-Syncthing-Version").decode("utf-8")
            if version:
...
class EventPollLoop(RESTRequest):
    ...
    def _response(self, stream, results):
        ...
        if headers is None:
            return
        headers = {x: y.strip() for (x, y) in [h.split(
            b":", 1) for h in headers if b":" in h]}
