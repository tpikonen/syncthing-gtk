...
class App(Gtk.Application, TimerManager):
    ...
    def __init__(self, gladepath="/usr/share/syncthing-gtk",
        ...
        self.send_limit = -1			# -//-
        # Used to prevent showing 'Do you want usage reporting'
        self.ur_question_shown = False
        # question more than once until ST-GTK is restarted.
        self.home_dir_override = None  # If set by '--home'
        self.wizard = None
        self.widgets = {}
        self.error_boxes = []
        # Holds set of already displayed error messages
    ...
    def setup_actions(self):
        ...
        add_simple_action(
            'daemon_output', self.cb_menu_daemon_output).set_enabled(False)
    ...
    def setup_statusicon(self):
        ...
        self.statusicon.connect(
            "notify::active", self.cb_statusicon_notify_active)
    ...
    def setup_connection(self):
        ...
        try:
            if self.home_dir_override:
                self.daemon = Daemon(os.path.join(
        ...
        self.daemon.connect("folder-rejected",
                            self.cb_syncthing_folder_rejected)
        self.daemon.connect("device-rejected",
                            self.cb_syncthing_device_rejected)
        ...
        self.daemon.connect("device-data-changed",
                            self.cb_syncthing_device_data_changed)
        ...
        self.daemon.connect(
            "device-paused", self.cb_syncthing_device_paused_resumed, True)
        self.daemon.connect(
            "device-resumed", self.cb_syncthing_device_paused_resumed, False)
        ...
        self.daemon.connect("folder-data-changed",
                            self.cb_syncthing_folder_data_changed)
        self.daemon.connect(
            "folder-data-failed", self.cb_syncthing_folder_state_changed, 0.0, COLOR_NEW, "")
        self.daemon.connect("folder-sync-started", self.cb_syncthing_folder_state_changed,
                            0.0, COLOR_FOLDER_SYNCING, _("Syncing"))
        self.daemon.connect(
        ...
        self.daemon.connect("folder-scan-started", self.cb_syncthing_folder_state_changed,
                            1.0, COLOR_FOLDER_SCANNING, _("Scanning"))
        self.daemon.connect(
    ...
    def start_daemon(self):
        if self.process is None:
            if IS_WINDOWS:
                ...
                if is_shutting_down():
                    log.warning(
                        "Not starting daemon: System shutdown detected")
                    return
            self.ct_process()
    ...
    def ct_process(self):
        ...
        vars, preargs, args = parse_config_arguments(
            self.config["syncthing_arguments"])
        cmdline = preargs + cmdline + args
        if self.home_dir_override:
            cmdline += ["-home", self.home_dir_override]

        self.process = DaemonProcess(
            cmdline, self.config["daemon_priority"], self.config["max_cpus"], env=vars)
    ...
    def ask_for_ur(self, *a):
        ...
        r.add_button(RIBar.build_button(
            "gtk-yes", use_stock=True), RESPONSE_UR_ALLOW)
        r.add_button(RIBar.build_button(
            "gtk-no", use_stock=True), RESPONSE_UR_FORBID)
    ...
    def check_for_upgrade(self, *a):
        ...
        if not can_upgrade_binary(self.config["syncthing_binary"]):
            self.cb_syncthing_error(
                None, "Warning: No write access to daemon binary; Skipping update check.")
            return
        # Determine platform
        ...
        def cb_cu_download_fail(sd, exception, message, r):
            ...
            r.close()
            self.cb_syncthing_error(
                None, _("Failed to download upgrade: %s") % (message))
            return cb_cu_error()
        ...
        def cb_cu_version(sd, version):
            ...
            try:
                needs_upgrade = not compare_version(
                    self.daemon.get_version(), version)
            except Exception:
            ...
            if needs_upgrade:
                ...
                self.timer("updatecheck", UPDATE_CHECK_INTERVAL,
    ...
    def swap_updated_binary(self):
        ...
        try:
            ...
            log.warning(
                "Failed to upgrade daemon binary: Failed to rename old binary")
            log.warning(e)
            return
        # Place new
        try:
            ...
            log.warning(
                "Failed to upgrade daemon binary: Failed to rename new binary")
            log.warning(e)
            ...
            try:
                ...
                log.error(
                    "Failed to upgrade daemon binary: Failed to rename backup")
                log.exception(e)
            return
        # Remove old
        try:
    ...
    def cb_syncthing_disconnected(self, daemon, reason, message):
        ...
        if reason == Daemon.SHUTDOWN:
            ...
            message = "%s %s..." % (
                _("Syncthing is restarting."), _("Please wait"))
        self.display_connect_dialog(
            message, quit_button=reason != Daemon.RESTART)
        if reason == Daemon.SHUTDOWN:
            # Add 'Start daemon again' button to dialog
            self.connect_dialog.add_button(
                "Start Again", RESPONSE_START_DAEMON)
        elif reason == Daemon.RESTART:
    ...
    def cb_syncthing_con_error(self, daemon, reason, message, exception):
        if reason == Daemon.REFUSED:
            ...
            if exception is not None and hasattr(exception, 'full_response'):
                ...
                swin.set_policy(Gtk.PolicyType.AUTOMATIC,
    ...
    def cb_syncthing_error(self, daemon, message):
        ...
        if "remote device speaks an older version of the protocol" in message:
            ...
            version = re.search(r"protocol \(([^\)]+)\)", message)
    ...
    def cb_syncthing_folder_rejected(self, daemon, nid, rid, label):
        ...
        if can_fix:
            r.add_button(RIBar.build_button(_("_Ignore")),
                         RESPONSE_FIX_IGNORE_FOLDER)
            r.add_button(RIBar.build_button(_("_Add")),
                         RESPONSE_FIX_ADD_FOLDER)
    ...
    def cb_syncthing_my_id_changed(self, daemon, device_id):
        if device_id in self.devices:
            ...
            device.add_value("inbps",	"dl_rate.svg",	_(
                "Download Rate"),		"0 B/s (0 B)")
            device.add_value("outbps",	"up_rate.svg",	_(
            ...
            for folder in self.folders:
                ...
                if device in f["devices"]:
                    f["shared"] = ", ".join([n.get_title()
                                            for n in f["devices"] if n != device])
    ...
    def cb_syncthing_device_data_changed(self, daemon, nid, address, client_version,
        ...
        if nid in self.devices:  # Should be always
            ...
            # Update rates
            device['inbps'] = "%s/s (%s)" % (sizeof_fmt(inbps),
                                             sizeof_fmt(inbytes))
            device['outbps'] = "%s/s (%s)" % (sizeof_fmt(outbps),
                                              sizeof_fmt(outbytes))
    ...
    def cb_syncthing_folder_added(self, daemon, rid, r):
        self.show_folder(
            ...
            r["fsWatcherEnabled"],
            sorted(
                [self.devices[n["deviceID"]]
                    for n in r["devices"] if n["deviceID"] in self.devices],
    ...
    def cb_syncthing_folder_data_changed(self, daemon, rid, data):
        if rid in self.folders:  # Should be always
            ...
            need_files = data["needFiles"] + data["needSymlinks"] + \
                data.get("receiveOnlyTotalItems", 0)
            need_bytes = data["needBytes"] + \
                data.get("receiveOnlyChangedBytes", 0)
            folder["global"] = "%s %s, %s" % (global_files, _(
                "Files"), sizeof_fmt(data["globalBytes"]))
            folder["local"] = "%s %s, %s" % (local_files, _(
                "Files"), sizeof_fmt(data["localBytes"]))
            folder["oos"] = "%s %s, %s" % (
                need_files, _("Files"), sizeof_fmt(need_bytes))
    ...
    def cb_syncthing_folder_up_to_date(self, daemon, rid):
        if rid in self.folders:  # Should be always
            ...
            self.cb_syncthing_folder_state_changed(
                daemon, rid, 1.0, COLOR_FOLDER_IDLE, title)
    ...
    def set_status(self, is_connected):
        ...
        if is_connected:
            if self.daemon.syncing():
                ...
                if len(sr) == 1:
                    self["menu-si-status"].set_label(
                        _("Synchronizing '%s'") % (sr[0],))
                else:
                    self["menu-si-status"].set_label(
                        _("Synchronizing %s folders") % (len(sr),))
                self.animate_status()
            elif self.any_device_online():
                # Daemon is online and idle
                self.statusicon.set("si-%s-idle" %
                                    (self.config['icon_theme'],), _("Up to Date"))
                self["menu-si-status"].set_label(_("Up to Date"))
                self.cancel_timer("icon")
            else:
                # Daemon is online, but there is no remote device connected
                self.statusicon.set(
                    "si-%s-unknown" % (self.config['icon_theme'],), _("All devices offline"))
                self["menu-si-status"].set_label(_("All devices offline"))
                self.cancel_timer("icon")
        else:
            # Still connecting to syncthing daemon
            self.statusicon.set(
                "si-%s-unknown" % (self.config['icon_theme'],), _("Connecting to Syncthing daemon..."))
            self["menu-si-status"].set_label(
                _("Connecting to Syncthing daemon..."))
            self.cancel_timer("icon")
    ...
    def animate_status(self):
        ...
        if self.timer_active("icon"):
            # Already animating
            return
        self.statusicon.set("si-%s-%s" %
                            (self.config['icon_theme'], self.sync_animation,))
    ...
    def display_connect_dialog(self, message, quit_button=True):
        ...
        if self.connect_dialog is None:
            ...
            self.connect_dialog.connect(
                "response", self.cb_connect_dialog_response, None)
            if self.is_visible():
    ...
    def display_run_daemon_dialog(self):
        ...
        if self.connect_dialog is None:  # Don't override already existing dialog
            ...
            # There is only one response available on this dialog
            self.connect_dialog.connect(
                "response", self.cb_connect_dialog_response, cb)
            if self.is_visible():
                self.connect_dialog.show_all()
            else:
    ...
    def show_folder(self, id, label, path, folder_type, ignore_perms, rescan_interval, fswatcher_enabled, shared):
        ...
        if id in self.folders:
            ...
            box.add_value("local",			"home.svg",		_(
                "Local State"),		"? items, ?B")
            box.add_value("oos",			"dl_rate.svg",	_(
                "Out Of Sync"),		"? items, ?B")
        ...
        box.set_visible(
            "id",		self.config["folder_as_path"] or label not in (None, ""))
    ...
    def show_device(self, id, name, compression, introducer, used):
        ...
        if id in self.devices:
            ...
            box.add_value("sync",		"sync.svg",		_(
                "Synchronization"),	"0%", visible=False)
            box.add_value("compress",	"compress.svg",	_("Compression"))
            box.add_value("inbps",		"dl_rate.svg",	_(
                "Download Rate"),		"0 B/s (0 B)", visible=False)
            box.add_value("outbps",		"up_rate.svg",	_(
                "Upload Rate"),		"0 B/s (0 B)", visible=False)
            box.add_value("introducer",	"thumb_up.svg",	_("Introducer"))
            box.add_value("version",	"version.svg",	_(
                "Version"),			None, visible=False)
            box.add_value('last-seen',	"clock.svg",
                          _("Last Seen"),			_("Never"))
            # Add hidden stuff
    ...
    def change_setting_async(self, setting_name, value, retry_on_error=False, restart=True):
        ...
        def csnr_error(e, trash, setting_name, value, retry_on_error):
            ...
            if retry_on_error:
                log.error("Retrying...")
                self.change_setting_async(
                    setting_name, value, retry_on_error, restart)
            else:
                log.error("Giving up.")
        ...
        def csnr_save_error(e, *a):
            ...
            """
            log.error(
                "change_setting_async: Failed to store configuration: %s", e)
            log.error("Giving up.")
        ...
        def csnr_config_read(config, setting_name, value, retry_on_error):
            ...
            if hasattr(value, '__call__'):
                ...
                c[setting] = value
            self.daemon.write_config(
                config, csnr_config_saved, csnr_save_error, setting_name, value)
        ...
        self.daemon.read_config(
            csnr_config_read, csnr_error, setting_name, value, retry_on_error)
    ...
    def quit(self, *a):
        if self.process is not None:
            if IS_WINDOWS:
                ...
                cb = Gtk.CheckButton(
                    _("Always do same; Don't show this window again"))
        ...
        def have_config(*a):
            ...
            for rid in self.folders:
                if self.folders[rid]["path"] == path:
                    ...
                    return
            log.warning(
                "Failed to remove directory for path '%s': No such folder", path)
    ...
    def cb_popup_menu_folder(self, box, button, time):
        ...
        if box["can_override"]:
            self["menu-popup-override"].set_visible(
                box["folder_type_s"] == "sendonly")
            self["menu-popup-revert"].set_visible(
                box["folder_type_s"] == "receiveonly")
    ...
    def cb_popup_menu_device(self, box, button, time):
        ...
        self["menu-popup-pause-device"].set_visible(
            box.get_status() != _("Paused"))
        self["menu-popup-resume-device"].set_visible(
            box.get_status() == _("Paused"))
    ...
    def cb_menu_popup_delete_folder(self, *a):
        ...
        # Editing folder
        self.check_delete(
            "folder", self.rightclick_box["id"], self.rightclick_box.get_title())
    ...
    def cb_menu_popup_delete_device(self, *a):
        """ Handler for other 'edit' context menu item """
        self.check_delete(
            "device", self.rightclick_box["id"], self.rightclick_box.get_title())
    ...
    def check_delete(self, mode, id, name):
        ...
        if r == Gtk.ResponseType.YES:
            ...
            self.daemon.read_config(
                self.cb_delete_config_loaded, None, mode, id)
    ...
    def cb_delete_config_loaded(self, config, mode, id):
        ...
        if mode == "folder":
            ...
            config["devices"] = [
                x for x in config["devices"] if x["deviceID"] != id]
            if id in self.devices:
    ...
    def open_editor_device(self, id=None, name=None):
        ...
        self.editor_device = DeviceEditorDialog(
            self, id not in self.devices, id)

        if name and id not in self.devices:
            self.editor_device.call_after_loaded(
                self.editor_device["vname"].set_text, name)
    ...
    def open_editor_folder(self, id=None, label=None, nid=None):
        ...
        self.editor_folder = FolderEditorDialog(
            self, id not in self.folders, id)

        # Find folder with matching ID ...
        if id in self.folders:
            # ... if found, show edit dialog and pre-select
            # matching device
            self.editor_folder.call_after_loaded(
                self.editor_folder.mark_device, nid)
        else:
            ...
            self.editor_folder.call_after_loaded(
                self.editor_folder.mark_device, nid)
            self.editor_folder.call_after_loaded(
                self.editor_folder.fill_folder_id, id)
            if label:
                self.editor_folder.call_after_loaded(
                    self.editor_folder["vlabel"].set_text, label)
    ...
    def cb_infobar_response(self, bar, response_id, additional_data={}):
        ...
        if response_id == RESPONSE_RESTART:
            ...
            if additional_data["nid"] in self.devices:
                self.open_editor_folder(
                    additional_data['rid'], additional_data['label'], additional_data['nid'])
            ...
            self.open_editor_device(
                additional_data['nid'], additional_data['name'])
    ...
    def cb_kill_daemon_response(self, dialog, response, checkbox):
        ...
        if checkbox.get_active():
            self.config["autokill_daemon"] = (
                1 if response == RESPONSE_SLAIN_DAEMON else 0)
        self.process = None
        self.cb_exit()
    ...
    def cb_wizard_finished(self, wizard, *a):
        ...
        if wizard.is_finished() and not self.exit_after_wizard:
            ...
            if self.process:
                self.process.kill()
                self.cb_daemon_exit(self.process, 0)
            if self.setup_connection():
                self.daemon.reconnect()
        else:
            self.quit()
    ...
    def cb_daemon_startup_failed(self, proc, exception):
        ...
        if d is None:
            ...
            self.config["syncthing_binary"] = self.config.get_default_value(
                "syncthing_binary")
            self.config.save()
            self.hide()
            self.show_wizard()
