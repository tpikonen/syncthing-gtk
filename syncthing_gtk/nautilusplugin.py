...
class NautiluslikeExtension(GObject.GObject):
    ...
    def __init__(self):
        ...
        self.daemon.connect(
            "folder-sync-started", self.cb_syncthing_folder_state_changed, STATE_SYNCING)
    ...
    def _is_ignored_path(self, fullpath):
        ...
        for ignorePath in sorted(self.ignore_paths[repo]):
            if path.startswith(ignorePath + os.path.sep):
                log.debug("Found %signore-path %s for %s" %
                          (("un" if not self.ignore_paths[repo][ignorePath] else "", ignorePath, path)))
        ...
        log.debug("Path %s is %s ignored" %
                  (path, "NOT" if not is_ignored else ""))
        return is_ignored
    ...
    def cb_syncthing_item_started(self, daemon, rid, filename, *a):
        ...
        if rid in self.rid_to_path:
            ...
            placeholderpath = os.path.join(
                path, ".syncthing.%s.tmp" % filename)
