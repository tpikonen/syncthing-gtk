...
class DaemonSettingsDialog(EditorDialog):
        ...
        if key == "listenAddresses":
            ...
            if value:
                if self.values["maxSendKbps"] <= 0:
                    ...
                    self.find_widget_by_id("vmaxSendKbps").get_adjustment(
                    ).set_value(self.values["maxSendKbps"])
            else:
            ...
            if value:
                if self.values["maxRecvKbps"] <= 0:
                    ...
                    self.find_widget_by_id("vmaxRecvKbps").get_adjustment(
                    ).set_value(self.values["maxRecvKbps"])
            else:
    ...
    def update_special_widgets(self, *a):
        ...
        self["vmaxRecvKbps"].set_sensitive(
            self.get_value("maxRecvKbpsEnabled"))
        self["lblvlocalAnnouncePort"].set_sensitive(
            self.get_value("localAnnounceEnabled"))
        self["vlocalAnnouncePort"].set_sensitive(
            self.get_value("localAnnounceEnabled"))
        self["lblvglobalAnnounceServers"].set_sensitive(
            self.get_value("globalAnnounceEnabled"))
        self["lblvglobalAnnounceServers"].set_sensitive(
            self.get_value("globalAnnounceEnabled"))
