...
def extract_number(x): return RE_NUMBER.match(x).group(1) if \
    RE_NUMBER.match(x) else "0"


def win32version(x): return ".".join(
...
if 'build' in sys.argv:
    for ll in wrong_sized_dll:
        print("replacing " + ll)
        shutil.copy(
            os.path.join(gnome_dll_path, ll),
            os.path.join(build_dir, ll)
        )
